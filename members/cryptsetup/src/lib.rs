...
impl CryptDevice {
    pub fn init(device: Option<&str>) -> Result<Self, CryptError> {
        let mut pointer = ptr::null_mut();
         ...
         unsafe {
            let status = crypt_init(&mut pointer, match device {
                Some(device) => as_cstr(device).as_ptr(),
                None => ptr::null()
            });

            if status == 0 { Ok(Self(pointer)) } else { Err(CryptError::Init) }
        }
    }
    ...
    pub fn get_uuid(&self) -> &str {
        unsafe { ptr_as_str(crypt_get_uuid(self.as_ptr())) }
    }
    ...
    pub fn get_data_offset(&self) -> u64 {
	       unsafe { crypt_get_data_offset(self.as_ptr()) }
	}

    pub fn get_iv_offset(&self) -> u64 {
    	unsafe { crypt_get_iv_offset(self.as_ptr()) }
	}
    ...
    pub fn get_sector_size(&self) -> u32 {
    	unsafe { crypt_get_sector_size(self.as_ptr()) as u32 }
	}

    pub fn as_ptr(&self) -> *mut crypt_device {
        self.0
    }
}
...
impl Drop for CryptDevice {
    fn drop(&mut self) {
        unsafe { crypt_free(self.as_ptr()) }
    }
}

fn as_cstr(input: &str) -> CString {
...
unsafe fn ptr_as_str<'a>(ptr: *const libc::c_char) -> &'a str {
    CStr::from_ptr(ptr)
        .to_str()
        .expect("cryptsetup returned invalid UTF-8")
}
